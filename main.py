...
class SkillManager:
    ...
    def get_or_create_kb_skill(kb_name: str, description: str) -> Optional[str]:
        ...
        skill_name = f"kb_skill_{kb_name}"
        try:
    ...
    def get_or_create_db_skill(db_name: str, description: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error with DB skill: {e}")
            return None

    @staticmethod
    def validate_database_exists(db_name: str) -> bool:
...
class StreamRouter:
    ...
    @app.get("/streams")
    def fetch_streams():
        """Fetch available streams from configured source"""
    ...
    def fetch_schema():
        """Fetch schema for selected streams"""
        if not session["source"]:
            return {"error": "Source not configured"}
    ...
    def create_agent_skills(data: AgentSkillsData):
        ...
        try:
            ...
            for kb_name in data.kb_names:
                try:
                    skill_name = SkillManager.get_or_create_kb_skill(kb_name, f"Knowledge from {kb_name}")
                    if skill_name:
            ...
            for db_name in data.db_names:
                try:
                    ...
                    if not SkillManager.validate_database_exists(db_name):
                        errors.append(f"Database '{db_name}' does not exist. Make sure to create it first.")
                        continue

                    skill_name = SkillManager.get_or_create_db_skill(db_name, f"SQL access to {db_name} database")
                    if skill_name:
                        new_skills.append(skill_name)
